...
function skillFactory<TData extends { [key: string]: any }>(
  ...
  for (let skillId in data) {
    ...
    if ('tier' in skillData) {
      ...
      (convertedSkill as IOccupationalSkill)['restrictedPurchase'] = skillData?.restrictedPurchase;
...
console.log('characterSkills', characterSkills);
console.log('occupationalSkills', occupationalSkills);
