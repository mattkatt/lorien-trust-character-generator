...
export const SkillButton: FC<ISkillButtonProps> = ({ skill}) => {
    ...
    const isDisabled = (): boolean => {
        ...
        const enoughSkillPoints = Helpers.isCharacterSkill(skill) ? skill.cost <= characterState.unspentCharacterSkillPoints : true;

        const meetsPrerequisites = skill?.prerequisites ? skill.prerequisites.some(prerequisite => {
    ...
    const onSkillClick = () => {
        if (isSelected()) {
             skillType === 'character' ? removeCharacterSkill(skill as ICharacterSkill) : removeOccupationalSkill(skill as IOccupationalSkill)
        } else {
            skillType === 'character' ? addCharacterSkill(skill as ICharacterSkill) : addOccupationalSkill(skill as IOccupationalSkill)
        }
    }
